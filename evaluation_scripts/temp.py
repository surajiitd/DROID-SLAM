...
def image_stream(datapath, modality, stride, is_sync, scaled_prgbd=False):
    ...
    #fx, fy, cx, cy = 517.3, 516.5, 318.6, 255.3 #TUM
    fx, fy, cx, cy = 481.20, -480.00, 319.50, 239.50 #ICL
    ...
    for t, (imfile, depth_file) in enumerate(zip(images_list,depth_list)):
        ...
        # so, no need to do below +32 and +16, if you are not doing the un-distortion.
        ...
        if modality == 'rgbd' or modality == 'p-rgbd':
            ...
            depth = F.interpolate(depth[None,None], (240, 320)).squeeze()

        intrinsics = torch.as_tensor([fx, fy, cx, cy]).cuda()
        intrinsics[0] *= image.shape[2] / 640.0 # =w0
        intrinsics[1] *= image.shape[1] / 480.0 # =h0
        intrinsics[2] *= image.shape[2] / 640.0 # =w0
        intrinsics[3] *= image.shape[1] / 480.0 # =h0
...
if __name__ == '__main__':
    ...
    seq = os.path.basename(args.datapath)
    gt_pose_file_name = ("livingRoom"+seq[16] if seq.startswith("living") else seq.split("_")[0] )+ ".gt.freiburg"
    #print("gt_pose_file_name = ",gt_pose_file_name)
    gt_file = os.path.join(args.datapath, gt_pose_file_name)
    ...
    def store_result_in_csv(args, sequence_name, res):
        ...
        if os.path.exists(path):
            ...
            if args.scaled_prgbd:
                df = pd.DataFrame(columns=["Time", "Depth Source", "Sequence name", "#Frames in seq","scale(mean,min,median,max)" "rmse", "max","mean","median","min","sse","std"])

        global depth_source
        # Add a new row to the dataframe
        row_to_be_added = {"Time": [datetime.now()], "Depth Source": depth_source, "Sequence name": [sequence_name], "#Frames in seq": [no_of_frames_processed],"rmse": [round(res['rmse'],6)],
        ...
        if args.scaled_prgbd:
            row_to_be_added["scale(mean,min,median,max)"]: (np.mean(scales),np.min(scales),np.median(scales),np.max(scales))

        row = pd.DataFrame(row_to_be_added)
